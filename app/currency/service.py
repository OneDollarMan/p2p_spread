from currency.models import Chain2, Pair, Chain2Reverse, Chain3, Chain3Reverse
...
def get_chain3_objects_filtered(form=None):
    if form is None:
        return Chain3.objects.filter(buy_pair__fiat__abbr='RUB').order_by('-profit')[:20]
    else:
        buy_payment = form['buy_payment'].value()
        sell_payment = form['sell_payment'].value()
        is_sell_orders = form['is_sell_orders'].value()
        objects = Chain3.objects.filter(buy_pair__fiat__abbr='RUB')
        try:
            if buy_payment:
                objects = objects.filter(buy_pair__payment_id=int(buy_payment))
            if sell_payment:
...
def get_chain3reverse_objects_filtered(form=None):
    if form is None:
        return Chain3Reverse.objects.filter(forward_chain__buy_pair__fiat__abbr='RUB').order_by('-profit')[:20]
    else:
        buy_payment = form['buy_payment'].value()
        sell_payment = form['sell_payment'].value()
        is_sell_orders = form['is_sell_orders'].value()
        objects = Chain3Reverse.objects.filter(forward_chain__buy_pair__fiat__abbr='RUB')
        try:
            if buy_payment:
                objects = objects.filter(forward_chain__buy_pair__payment_id=int(buy_payment))
            if sell_payment:
